...
def get_parameter_curve_fit(flux, x, y, profile,beam):
    """
    ...
    if profile == 'A':
        ...
        initial_guess = (0, 0, bmin_s, bmaj_s-bmin_s, 1, theta, 1, 0.3, 0.5, theta)
        popt, pcov = opt.curve_fit(Gaussian_sersic, (x, y), flux.ravel(), p0=initial_guess,
                                   bounds=((-0.2, -0.2, 0, 0, 0.0001, 0.,0,0,0,0),
...
def log_likelihood(theta, x, y, z, zerr, profile):
    """
    ...
    if profile == 'A':
        ...
        model = model_twoD_Gaussian(theta1, x, y) + model_Sersic(theta2, x, y)

    return -np.power((z - model), 2) / (2 * zerr ** 2) - 0.5 * np.log(2 * np.pi * zerr ** 2)


def lnprior(theta, curve_fit_mu_sigma, bmin, bmaj, profile, ):
    """
    ...
    if profile == 'A':
        ...
        if not np.all(amplitude > 0 and sigma_x > 0 and 0 < sigma_y <= sigma_x and 0 <= angle <= 180 and -0.2<x_0<0.2 and -0.2<y_0<0.2):
            return -np.inf
        # mu1 = 17  # curve_fit_mu_sigma[0]
        # # sigma1 = curve_fit_mu_sigma[1] * 10
        # sigma1 = 2
        ...
        amplitude, x_0, y_0, sigma_x, sigma_y, angle, amplitude_sersic, r_eff, ellip, angle_sersic = theta
        if not np.all(
                amplitude>0 and -0.2 < x_0 < 0.2 and -0.2 < y_0 < 0.2 and 0 < sigma_x <= bmaj and sigma_y <= sigma_x and 0 < sigma_y <= bmin and 0 < angle <= 180 and
...
def log_probability(theta, x, y, z, zerr, curve_fit_mu_sigma, bmin, bmaj, profile):
    # log(likelihood * priors) = loglikelihood + log priors

    lp = lnprior(theta, curve_fit_mu_sigma, bmin, bmaj, profile)
    if not np.isfinite(lp):
        return -np.inf#, -np.inf * np.ones(34*34)
    log_likelihood_ = log_likelihood(theta, x, y, z, zerr, profile)
